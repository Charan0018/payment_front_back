...
var state = {
  ...
  'rows': 11,
	...
	fetch('http://localhost:8080/Summer_Internship_Backend/dummy.do/*')
	  .then((response) => {
...
document.getElementById("addsub").addEventListener("submit", (event) => {
	  ...
	  fetch(`http://localhost:8080/Summer_Internship_Backend/add.do/?Name=${customer_name}&Customer=${customer_number}&Amount=${invoice_amount}&Invoice=${invoice_id}&due=${due_date}&notes=${notes}`,
...
document.getElementById("editform").addEventListener("submit", (event) => {
	...
	fetch(`http://localhost:8080/Summer_Internship_Backend/edit.do/?Amount=${invoice_amount}&Invoice=${invoice_id}&notes=${notes}`,
			  {method:"POST"}).then(response => {
		  return response.text();
	  }).then(x => {
		  location.reload();
...
document.getElementById("delsub").addEventListener("submit", (event) => {
	...
	parIds.forEach((id) => {
		fetch(`http://localhost:8080/Summer_Internship_Backend/delete.do/?Invoice=${id}`,
